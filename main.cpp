...
int main()
{
    ...
    float spawnDelay = 1.0f; // 1.0f intial spawn logic so that enemies are always coming even after score = 50.
    ...
    while (!WindowShouldClose())
    {
        ...
        if (current_screen == 1)
        {
            ...
            // Update + Draw player functions call.

            player.update();
            player.Draw();

            // Update + Draw enemies functions call.
...
void ResetGame(Player &player, vector<Enemy *>& enemies, float &spawnTimer1)
{
    player.Reset(); // Implement this in your Player class
    for (auto &enemy : enemies)
    {
        delete enemy;
    }
